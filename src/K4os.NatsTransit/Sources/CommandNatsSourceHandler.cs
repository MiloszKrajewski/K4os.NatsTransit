    ...
    where TCommand: IRequest
{
    ...
    private readonly INatsDeserialize<TCommand> _requestDeserializer;
    ...
    public CommandNatsSourceHandler(NatsToolbox toolbox, Config config)
    {
        ...
        _requestDeserializer = toolbox.Deserializer<TCommand>();
    }
    ...
    private async Task ConsumeOne(
        NatsJSMsg<TCommand> message, MessageHandler handler, CancellationToken token)
    {
        try
        {
    ...
    private static Task TryExecuteHandler(
        MessageHandler handler, TCommand request, CancellationToken token) =>
